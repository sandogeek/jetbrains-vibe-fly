...
pnpm --filter @vibefly/ui run build      # 输出到 ../vibefly-jcef/src/main/resources/web
...
**Run UI Dev** 走 Gradle 任务 `runVibeflyUiDev`（前台 `pnpm run dev`，日志在 IDE Run / Gradle 控制台）。
...
Gradle 在 `:vibefly-jcef:processResources` 前会执行 `buildVibeflyUi`（`pnpm run build`）。若尚未 `pnpm install`，该任务会跳过构建并打日志。
