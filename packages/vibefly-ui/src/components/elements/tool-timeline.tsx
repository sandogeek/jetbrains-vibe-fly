...
export interface ToolTimelineProps {
    ...
    stats: TimelineStat[]
    className?: string
    /** When set, replaces the verb/chip list. File stats still render below. */
    children?: ReactNode
}
        ...
        >
            ...
            <CollapsibleContent className={cn(collapsePanel, "outline-none")}>
                <div className="flex flex-col gap-2.5 ps-4 pt-2.5">
                    {children ??
                        take(steps, visibleSteps).map((step, stepIndex, shown) => {
                            const Icon = step.icon
                            const active = streaming && stepIndex === shown.length - 1
                            return (
                                <div
                                    key={stepIndex}
