...
function SelectTrigger({className, children, ...props}: ComponentProps<typeof SelectPrimitive.Trigger>) {
    return (
        <SelectPrimitive.Trigger
            className={cn(
                "flex h-9 w-full items-center justify-between gap-2 rounded-md border border-input bg-surface px-2 py-1.5 text-sm text-fg ring-offset-bg",
                "placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
    ...
    return (
        <SelectPrimitive.Portal>
            <SelectPrimitive.Content
                className={cn(
                    "relative z-[110] max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border bg-bg text-fg shadow-md",
                    position === "popper" &&
