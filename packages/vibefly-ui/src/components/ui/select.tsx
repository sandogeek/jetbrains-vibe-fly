...
const SelectTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <SelectPrimitive.Trigger
      class={cn(
        "flex h-9 w-full items-center justify-between rounded-md border border-input bg-surface px-2 py-1.5 text-sm text-fg ring-offset-bg placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
        local.class,
      )}
      {...others}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          "relative z-[110] min-w-32 overflow-hidden rounded-md border border-border bg-bg text-fg shadow-md outline-none",
          local.class,
        )}
        {...others}
      >
...
const SelectItem = <T extends ValidComponent = "li">(
  ...
  return (
    <SelectPrimitive.Item
      class={cn(
        "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-surface data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        local.class,
      )}
      {...others}
    >
...
const SelectLabel = <T extends ValidComponent = "label">(
  ...
  return (
    <SelectPrimitive.Label
      class={cn(labelVariants({ variant: "label" }), "text-muted", local.class)}
      {...others}
    />
  )
}
