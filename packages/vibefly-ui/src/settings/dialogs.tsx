...
export function CustomProviderDialog({existing, catalogIds, existingCustomIds, onClose}: CustomDialogProps) {
    ...
    const [modelsText, setModelsText] = useState(existing?.models ? formatModelsText(existing.models) : "");
    const [apiKey, setApiKey] = useState("");
    const [error, setError] = useState<string | null>(null)
    const submit = () => {
        const err = validateProviderId(id, catalogIds, existingCustomIds, isEdit, providerUiLabels(t));
    ...
    return <ModalShell title={isEdit ? t("dialogs:editProvider", {name: existing!.id}) : t("dialogs:addCustomProvider")}
                       onCancel={() => onClose({kind: "cancel"})}>
        <TextField value={id} onChange={setId}
                   disabled={isEdit}><TextFieldLabel>{t("dialogs:providerId")}</TextFieldLabel><TextFieldInput
            className="font-mono"/></TextField>
        <TextField value={baseUrl}
                   onChange={setBaseUrl}><TextFieldLabel>{t("dialogs:baseUrl")}</TextFieldLabel><TextFieldInput
            placeholder="https://api.example.com/v1"/></TextField>
        <div className="mb-3"><label className="mb-1 block text-xs text-muted">{t("dialogs:api")}</label><select
            className="flex h-9 w-full rounded-md border border-input bg-surface px-2 py-1.5 text-sm text-fg"
            value={api} onChange={(event) => setApi(event.currentTarget.value)}>
            <option value="openai-completions">openai-completions</option>
            <option value="openai-responses">openai-responses</option>
            <option value="anthropic-messages">anthropic-messages</option>
            <option value="google-generative-ai">google-generative-ai</option>
        </select></div>
        <TextField value={modelsText}
                   onChange={setModelsText}><TextFieldLabel>{t("dialogs:modelsLine")}</TextFieldLabel><TextFieldTextArea
            className="h-28"/></TextField>
        <TextField value={apiKey}
                   onChange={setApiKey}><TextFieldLabel>{t("dialogs:apiKey")}</TextFieldLabel><TextFieldInput
            type="password" placeholder={t("dialogs:keepExistingKey")}/></TextField>
        {error && <p className="m-0 mb-2 text-xs text-destructive">{error}</p>}
        <DialogFooter><Button variant="outline"
                              onClick={() => onClose({kind: "cancel"})}>{t("common:cancel")}</Button><Button
            onClick={submit}>{t("common:save")}</Button></DialogFooter>
    </ModalShell>
}
...
function ModalShell({title, onCancel, children}: { title: string; onCancel: () => void; children: ReactNode }) {
    return <Dialog open onOpenChange={(open) => {
        if (!open) onCancel()
    }}><DialogContent showCloseButton={false}><DialogHeader><DialogTitle>{title}</DialogTitle></DialogHeader>{children}
