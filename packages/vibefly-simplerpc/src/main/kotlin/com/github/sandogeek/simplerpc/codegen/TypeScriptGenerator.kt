 ...
 *
 * - [@TsCallKotlin]: `ui2Host` definition, `Ui2Host` client type, `createUi2HostProxy(peer)`
 * - [@KotlinCallTs]: `host2Ui` definition, `Host2UiService` type, `registerHost2UiService(peer, impl)`
 *
 * Output uses [defineRpcService]/[rpcMethod] (no arity descriptors). Call options must
 * use branded `rpcOptions({...})` on the TypeScript side.
 */
...
object TypeScriptGenerator {
    ...
    private fun emit(models: List<InterfaceModel>, options: TypeScriptGenerationOptions): String {
        ...
        sb.appendLine()
        sb.appendLine(
            "import type { RpcClient, RpcService, SimpleRpcPeer } from \"${options.runtimeModule}\";",
        )
        sb.appendLine(
            "import { defineRpcService, rpcMethod } from \"${options.runtimeModule}\";",
        )
        sb.appendLine()
    ...
    private fun emitTsCallKotlin(sb: StringBuilder, model: InterfaceModel) {
        ...
        val defName = definitionConstName(name)
        sb.appendLine("export const $defName = defineRpcService(\"${escapeTsString(model.serviceName)}\", {")
        for (m in model.methods) {
            val argsTuple = methodArgsTuple(m)
            val ret = if (m.returnIsUnit) "void" else m.returnTs
            sb.appendLine(
                "  ${tsPropertyKey(m.tsName)}: rpcMethod<$argsTuple, $ret>(${m.id}),",
            )
        }
        sb.appendLine("});")
        sb.appendLine()
        sb.appendLine("export type $name = RpcClient<typeof $defName>;")
        sb.appendLine()
        sb.appendLine("export function create${name}Proxy(peer: SimpleRpcPeer): $name {")
        sb.appendLine("  return $defName.createProxy(peer);")
        sb.appendLine("}")
    }
    ...
    private fun emitKotlinCallTs(sb: StringBuilder, model: InterfaceModel) {
        ...
        sb.appendLine("});")
        sb.appendLine()
        sb.appendLine("export type $serviceType = RpcService<typeof $defName>;")
        sb.appendLine()
        sb.appendLine(
            "export function register${name}Service(" +
                "peer: SimpleRpcPeer, implementation: $serviceType) {",
        )
        sb.appendLine("  return $defName.register(peer, implementation);")
        sb.appendLine("}")
    }

    /** Ui2Host → ui2Host; Host2Ui → host2Ui. */
    private fun definitionConstName(typeName: String): String {
        if (typeName.isEmpty()) return "api"
        return typeName[0].lowercaseChar() + typeName.substring(1)
    }

    private fun methodArgsTuple(m: MethodModel): String {
        if (m.params.isEmpty()) return "[]"
        val parts = m.params.joinToString(", ") { "${it.name}: ${it.tsType}" }
        return "[$parts]"
    }
