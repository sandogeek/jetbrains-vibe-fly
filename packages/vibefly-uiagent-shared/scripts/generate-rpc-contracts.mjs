...
/** @returns {{ decorator: ts.Decorator, args: readonly ts.Expression[] } | null} */
...
function readServiceName(sourceFile, className, serviceDecorator) {
  ...
  if (!ts.isStringLiteral(arg) && !ts.isNoSubstitutionTemplateLiteral(arg)) {
    fail(
      sourceFile,
      arg,
      "@rpcService name must be a string literal",
    )
  }
  return arg.text
}

function readRpcId(sourceFile, methodName, idDecorator) {
...
export function parseRpcContracts(source, fileName = "contracts.ts") {
...
function sourceModulePath(inputPath, outputPath) {
...
export function emitRpcContracts(contracts, contractModule = "./contracts.js") {
  const lines = [
    "/* eslint-disable */",
...
export function generateRpcContracts(inputPath, outputPath, check = false) {
...
function main(args) {
  ...
  if (paths.length > 2) {
    throw new Error("Usage: generate-rpc-contracts.mjs [input] [output] [--check]")
  }
...
const entryPath = process.argv[1] == null ? null : pathToFileURL(resolve(process.argv[1])).href
