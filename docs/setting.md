...
- 设置 Tab 的 Host / Agent 操作显式绑定该 Tab 所属 Project，不再回退到“任取一个已打开 Project”。Provider 登录生命周期继续使用 `withControlForSettings` 和反向 RPC；没有迁移成独立 settings Agent。Provider 配置和 revision 收敛已进入统一 UI client。
